...
# 댄드로그램으로 군집 확인 
? hclust # ward.D", "ward.D2", "single", "complete", "average" (= UPGMA), "mcquitty" (= WPGMA), "median" (= WPGMC) or "centroid" (= UPGMC)
hc_res<-hclust(dist(xy)^2, method = "single" )
hc_res
plot(hc_res, hang = -1)
...
d <- dist(body[, -1], method="euclidean") # 번호 제외
d
...
# 1, 3 그릅이 안경유무에 의해 분류되는 것을 알 수 있고, 기타 세부적으로 요약 통계량에 의해 군집의 유사성을 확인할 수 있다.
