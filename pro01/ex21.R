...
# 분류 모델 작성 전 dataset을 train/test로 분리 7:3으로
set.seed(123)
idx=sample(1:nrow(iris),nrow(iris)*0.7)
train=iris[idx,] #idx행의 모든 열
test=iris[-idx,]
nrow(train) # 105
nrow(test) # 45

# 독립변수에 대한 정규화를 실시 (대개의 경우 정규화 or 표준화를 하면 모델의 성능이 향상)
...
model <- neuralnet(Species2 ~., data = train_nor, hidden = 1, algorithm = 'backprop', learningrate = 0.01)
model
plot(model)
